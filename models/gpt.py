...
class Gaussian_classfication(nn.Module):
    def __init__(self, im_dim):
        super().__init__() 
        self.clf=nn.Linear(128, 9)

        self.mu=nn.Sequential(       
        nn.Linear(im_dim* 1, 128))
    ...
    def forward(self, img_emb): #img_emb BxNxD
        x = torch.flatten(img_emb, start_dim=1) #B X N*D
        mu=self.mu(x) #batch_size*128
        logvar=self.logvar(x) #batch_size*128
        x=self._reparameterize(mu,logvar) #batch_size*128
        out=self.clf(x) #batch_size*9
        return out 

class Gaussian_prefix(nn.Module):
    def __init__(self, im_dim):
        super().__init__() 
        self.clf=nn.Linear(im_dim, 9)
        self.re = nn.ReLU(inplace=True)
        self.ln = nn.Linear(im_dim*2, im_dim)
    ...
    def forward(self, img_emb, control_emb): 
        x = torch.cat((control_emb,img_emb), dim=1)
        x=self.ln(x)
        out=self.clf(self.re(x))
        return x, out
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def gumbel(self,pre_emo,EMOID_TO_WORDID):
        tau = 0.5 
        gumbel_out = F.gumbel_softmax(pre_emo, tau, hard=True) 
        emo_keys = list(EMOID_TO_WORDID.keys())
        emo_ids = torch.tensor(emo_keys, dtype=torch.int64)
        selected_emo_ids = torch.matmul(gumbel_out, emo_ids.cuda().float())
        word_ids = [EMOID_TO_WORDID[int(emo_id)] for emo_id in selected_emo_ids]
    ...
    def forward(
        ...
        if token_type_ids is not None:
            # print('gpt_token_type_ids:',token_type_ids.shape, 'gpt_input_shape:',input_shape)
        ...
        word_embeds = self.wte(input_ids) 
        ...
        hidden_states[:,0,:] = control_embedding # inject the control embedding
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            outputs = block(
                ...
                encoder_hidden_states=encoder_hidden_states, # image embedding
                encoder_attention_mask=encoder_attention_mask,
                use_cache=use_cache,
                output_attentions=output_attentions,
            )
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.Gaussian_prefix_vis = Gaussian_prefix(self.encoder_dim)
        self.Gaussian_prefix_exp = Gaussian_prefix(self.encoder_dim)
    ...
    def Gaussian_regular(self, img_mu, img_logvar):
    ...
    def forward(
        ...
        prefix_vis, pre_emo_vis = self.Gaussian_prefix_vis(encoder_hidden_states[0], control_embedding)
        new_img = torch.cat((prefix_vis.unsqueeze(1),encoder_hidden_states[1]),dim=1)
        encoder_hidden_states = (encoder_hidden_states[0],) + (new_img,)
        ...
        if collect_vectors and lang_embeded.shape[1]>6:
            prefix_exp, pre_emo_exp = self.Gaussian_prefix_exp(exp_embeded, control_embedding)
            self.prefix_exp_list = prefix_exp.detach().cpu().numpy()
            self.exp_embeded_list = exp_embeded.detach().cpu().numpy()
        if labels is not None:
            ...
            loss_rec = 0.2*self.loss_mse(prefix_vis, prefix_exp)
            loss_emo = loss_fct(pre_emo_vis.contiguous(), emo_labels.contiguous()) + loss_fct(pre_emo_exp.contiguous(), emo_labels.contiguous())


            loss = loss + loss_xe_text + loss_rec + loss_emo
        loss_xe=[]
        ...
        return CausalLMOutputWithCrossAttentions(
            loss=(loss,loss_xe,loss_exp_contrasive,None),
