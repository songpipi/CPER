...
from models.emo_level_encoder import ControlEncoder
...
EMOTION_TO_IDX = {e: i for i, e in enumerate(ARTEMIS_EMOTIONS)} # amusement: 0

EMOID_TO_WORDID = {0:(30184,780), 1: (25030,780), 2: (2695, 434), 3: (14067,780), 4: (8993,780), 5: (16234,780), 6: (3252,780), 7: (25303,780), 8: (1223, 2073) }
...
def save_checkpoint(name, epoch, unwrapped_model, optimizer, tokenizer, scheduler, ckpt_path, best_acc, best_res, **kwargs):
    ...
    unwrapped_model.decoder.save_pretrained(ckpt_path + model_name, save_function=accelerator.save, safe_serialization=False)
    if opt.fixed_encoder: 
        ckpt_load = {'epoch': epoch,
        'optimizer_state_dict': optimizer.state_dict(), 
        'control_encoder_state_dict':unwrapped_model.control_encoder.state_dict(),
...
def sample_sequences(model, tokenizer, loader):
    ...
    for i,batch in  tqdm (enumerate(loader),total=len(loader)):
        ...
        img, img_id, input_ids, segment_ids, _ = batch
        # print('test_input:',input_ids.shape, 'test_segment:', segment_ids.shape)
        bt = img.size(0)
        ...
        with torch.no_grad():
            ...
            control_emb = model.control_encoder(bt, None, 'eval')
            # img_embeddings = (control_emb,) + (img_embeddings[1],)
...
def get_optimizer(model, learning_rate):
    ...
    if opt.fixed_encoder:
        ...
        {'params':[p for n, p in model.control_encoder.named_parameters() if not any(nd in n for nd in no_decay)],'weight_decay': weight_decay, 'lr':learning_rate[2]},
        {'params': [p for n, p in model.control_encoder.named_parameters() if any(nd in n for nd in no_decay)],'weight_decay': 0.0,'lr':learning_rate[2]},             
...
control_encoder = ControlEncoder(opt.bucket_path, opt.control_signal_at_inference, opt.prefix_dim)
control_encoder = control_encoder.to(device)
...
if opt.load_from_epoch is not None:
    ...
    tokenizer = GPT2Tokenizer.from_pretrained('gpt2/pretrain_tokenizer_0')
    orig_num_tokens = len(tokenizer.encoder)  
    num_new_tokens = tokenizer.add_special_tokens({'pad_token': '<pad>',
                                                    'additional_special_tokens': ['<prefix>','<emotion>','<explanation>']})
    assert len(tokenizer) == orig_num_tokens + 4  
    config = AutoConfig.from_pretrained('gpt2/pretrain_model')  
    # Add configs
    ...
    decoder_model = GPT2LMHeadModel.from_pretrained('gpt2/pretrain_model', config = config)
    decoder_model.resize_token_embeddings(len(tokenizer))
    decoder_model = decoder_model.to(device)
    
    model = nn.ModuleDict({'encoder': image_encoder, 'decoder': decoder_model, 'control_encoder':control_encoder})
    optimizer = get_optimizer(model, [opt.learning_rate,opt.en_LR,opt.emo_en_LR])
...
for epoch in range(start_epoch, opt.num_train_epochs):
    ...
    for step, batch in enumerate(train_loader):
        batch = tuple(input_tensor.to(device) for input_tensor in batch)
        img, _, input_ids, labels, segment_ids,_,emotion_distribution,emo_labels = batch
        ...
        emotion_distribution = emotion_distribution.view(-1,1) #[bt*2, 1]
        emo_labels = emo_labels.view(-1)
        ...
        if opt.fixed_encoder:
            ...
            img_embeddings = model.encoder(img) 
        control_emb = model.control_encoder(None,emotion_distribution, 'train')
        ...
        outputs = model.decoder(input_ids=input_ids, 
                    ...
                    return_dict=True,
                    alpha = opt.alpha,
                    emo_features = ef_output, #b*2, 30, 3
                    control_embedding = control_emb,
                    emo_ids=EMOID_TO_WORDID,
                    )
        ...
        if step % 1000 == 0:
            print(f"Epoch {epoch}/{opt.num_train_epochs}, Step {step}/{len(train_loader)}, "
                f"Accumulated Loss:{accum_loss}, "
                f"Accumulated XE Loss:{accum_loss_xe_text} ")
